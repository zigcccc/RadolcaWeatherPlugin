export default class API {
	constructor(lat, lon, lang) {
		(this.api_base =
			...
			(this.iconURL = 'https://radolca.si/media/Vreme/');
	}

	currentWeather() {
		return `${this.api_base}weather?lat=${this.lat}&lon=${this.lon}&appid=${
			this.appID
		}&lang=${this.lang}`;
	}
	forecast() {
		return `${this.api_base}forecast/?lat=${this.lat}&lon=${this.lon}&appid=${
			this.appID
		}&lang=${this.lang}`;
	}
